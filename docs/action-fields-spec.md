...
Payment confirmation actions are **only available for Bank Transfer**, because automated payment methods (Card Instalments, DD Instalments, DD Pay Full, Premium Credit) detect payments via Stripe webhooks, direct debit systems, or 3rd-party API callbacks. The admin cannot manually confirm payment receipt for these methods.

| Restricted Action | Available For | Reason |
| --- | --- | --- |
| Payment Received | Bank Transfer only | Other methods auto-detect via system |
| Final Payment | Bank Transfer only | Other methods auto-detect via system |
| Mark as Paid | Bank Transfer only | Other methods auto-collect via DD/system |
...
| Action | Target | Required Fields | Method Restriction | Notes |
| --- | --- | --- | --- | --- |
| Mark as Paid | Payment_Complete | **Reference Code** (auto-populated, editable) + Notes | **Bank Transfer only** | DD auto-collects. Balance amount shown as read-only context. |
...
| Reverse Refund | `cancellation_source_state` | Notes | Returns to original state before cancellation |
